## <img src="https://flagcdn.com/w40/us.png" width="40"> 🧩 St-Sync Plugin to integrate two [GLPI](https://www.glpi-project.org) instances via API.

### This is the latest plugin developed by the Brazilian company [Service TIC Soluções Tecnológicas LTDA](https://servicetic.com.br/links).
This plugin provides the ability to integrate two GLPI instances via API, allowing all updates in follow-ups, tasks, and solutions to be automatically reflected between both databases.
With ST-Sync, information synchronization between different GLPI environments becomes simple and efficient, providing a more integrated and seamless IT service management experience.

---
### ▶️ Step-by-step video
- [Click here](https://www.youtube.com/watch?v=q0zE0dcFktA&t)
---

### Configuration screens and results:

![image](https://github.com/serviceticst/st-sync/assets/70910492/4c4f5040-d4c3-4362-ae83-505f4ff9c047)

![image](https://github.com/serviceticst/st-sync/assets/70910492/15a83e8c-8d6b-4bd6-a1ae-b757c6567e8a)
#
![image](https://github.com/serviceticst/st-sync/assets/70910492/b2a8a4dd-b069-4f2a-a249-55ed9360bc48)

![image](https://github.com/serviceticst/st-sync/assets/70910492/cfc27de5-2279-468f-94c5-497d2691eec4)

---

### 📥 Download
- [Click here](https://github.com/serviceticst/st-sync/releases)
---

### ⚙️ Features
- Bidirectional synchronization via API between two GLPI instances
- Automatic replication of follow-ups
- Synchronization of tasks linked to tickets
- Real-time updates
- Secure authentication via API Token

***

### Developed by: Service TIC Soluções Tecnológicas

- **E-mail**: [contato@servicetic.com.br](mailto:contato@servicetic.com.br)
- **Website**: [www.servicetic.com.br](http://www.servicetic.com.br)
- **LinkedIn**: [@serviceticst](https://www.linkedin.com/company/serviceticst)
- **Instagram**: [@serviceticst](https://www.instagram.com/serviceticst)
- **Facebook**: [@serviceticst](https://www.facebook.com/serviceticst)
- **X (Twitter)**: [@serviceticst](https://x.com/serviceticst)
- **YouTube**: [@serviceticst](https://youtube.com/c/serviceticst)
- **WhatsApp Channel**: [Click here](https://whatsapp.com/channel/0029VaAkV3P59PwXAiDepu3N)

[![image](https://github.com/user-attachments/assets/17192a13-f0b6-4531-add0-99c7f46c24b0)](https://servicetic.com.br/links/)
